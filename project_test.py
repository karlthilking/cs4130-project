...
class Model:
  ...
  def csv_to_df(self, csv_path: str) -> pd.DataFrame:
    df = pd.read_csv(csv_path)
    return df
  
  def load_dataset(self, df_name: str) -> Union[pd.DataFrame, None]:
  ...
  def generate_code(self, df: pd.DataFrame, df_name: str, request: str) -> Union[str, None]:
  ...
  def execute_code(self, code, df) -> Union[pd.DataFrame, None, Any]:
    locals = {}
    globals = {
      'plt': plt,
      'pd': pd,
  ...
  def generate_summary(self, df, df_name, request) -> Union[str, None]:
  ...
  def main(self):
    df, df_name = None, None
    while df is None:
      datasets = [f'{i+1}. {k}' for i, k in enumerate(self.datasets.keys())]
      choice = input(f"\nSelect a dataset from the following: {' '.join(datasets)} (Press 1 for 1st dataset, 2 for 2nd, etc.):\n")
      if not choice.isdigit():
        print(f'Your selection could not be identified. Please try again.\n')
        continue
      choice = int(choice)
      if choice == 1 or choice == 2:
        df_name = list(self.datasets.keys())[choice - 1]
        df = self.csv_to_df(self.datasets[df_name])
        print(f"You've selected the {df_name} dataset!\n")
      else:
        print(f'Selection out of range. Pleas try again.\n')
    
    print(f'Here are the categories within your dataset:\n{", ".join(list(df.columns))}\n')
    while True:
      request = input("Enter any data categories you are interested in and a plot type for visualization (press q to exit):\n")
      if request.lower() == 'q':
        break

      code = self.generate_code(df, df_name, request)
      stats = self.execute_code(code, df)

      if stats is not None:
        print(f'Here are some key statistics:\n{stats}\n')
      else:
        print('Failed to generate stats.\n')
      
      summary = self.generate_summary(df, df_name, request)
      if summary:
        print(f'Summary:\n{summary}\n')
